...
<#macro userProfileFormFields>
	...
	<#list profile.attributes as attribute>
		...
		<#assign fieldLabel=advancedMsg(attribute.displayName!'')>
		<#if !fieldLabel?has_content>
			<#-- Если displayName пустое, используем имя атрибута или стандартные названия -->
			<#if attribute.name == "middleName">
				<#assign fieldLabel="Отчество">
			<#elseif attribute.name == "secondName" || attribute.name == "second_name">
				<#assign fieldLabel="Второе имя">
			<#else>
				<#assign fieldLabel=attribute.name>
			</#if>
		</#if>
		<@field.group name=attribute.name label=fieldLabel error=kcSanitize(messagesPerField.get('${attribute.name}'))?no_esc required=attribute.required>
			<div class="${properties.kcInputWrapperClass!}">
				...
				<#if attribute.annotations.inputHelperTextAfter??>
                  <#if attribute.name == "middleName">
                    <div class="${properties.kcInputHelperTextAfterClass!}" id="form-help-text-after-${attribute.name}" aria-live="polite" style="display: flex; align-items: center;">
                      <input type="checkbox" id="noMiddleNameCheckbox" style="margin-right: 4px;" />
                      <input type="hidden" id="noMiddleName" name="noMiddleName" value="false" />
                      <span>${kcSanitize(advancedMsg(attribute.annotations.inputHelperTextAfter))?no_esc}</span>
                    </div>
                    <script>
                      document.addEventListener('DOMContentLoaded', function() {
                        var checkbox = document.getElementById('noMiddleNameCheckbox');
                        var middleNameInput = document.getElementById('middleName');
                        var noMiddleNameInput = document.getElementById('noMiddleName');
                        if (checkbox && middleNameInput && noMiddleNameInput) {
                          checkbox.addEventListener('change', function() {
                            if (checkbox.checked) {
                              middleNameInput.value = '';
                              middleNameInput.disabled = true;
                              noMiddleNameInput.value = 'true';
                              middleNameInput.style.background = '#e0e0e0';
                              middleNameInput.style.color = '#555';
                            } else {
                              middleNameInput.disabled = false;
                              noMiddleNameInput.value = 'false';
...
<#macro inputTag attribute value>
	...
	<#if attribute.annotations.inputTypePlaceholder??>
		<#assign placeholderText=advancedMsg(attribute.annotations.inputTypePlaceholder)>
	<#else>
		<#-- Автоматические плейсхолдеры на основе имени поля -->
		<#if attribute.name == "username" || attribute.name == "email">
			<#assign placeholderText="Электронная почта">
		<#elseif attribute.name == "firstName">
			<#assign placeholderText="Имя">
		<#elseif attribute.name == "lastName">
			<#assign placeholderText="Фамилия">
		<#elseif attribute.name == "middleName">
	...
	<#if attribute.name == "phone">
		...
		<#-- Visible composite phone field:
		     1) Region selector (intl-tel-input on hidden text field)
		     2) Area code (3 digits inside parentheses)
		     3) Main number with mask ___-__-__ -->
		<span class="${properties.kcInputClass} <#if error?has_content>${properties.kcError}</#if>">
			<div class="kc-phone-input-row">
				<div class="kc-phone-region">
					<input
						type="tel"
						id="phone-visible"
						class="${properties.kcInputClass!} kc-phone-hidden-input"
						aria-label="Выбор региона для телефона"
						aria-invalid="<#if messagesPerField.existsError('${attribute.name}')>true</#if>"
						<#if attribute.readOnly>disabled</#if>
						data-phone-visible="1"
					/>
				</div>
				<div class="kc-phone-area">
					<span class="kc-phone-area-bracket kc-phone-area-bracket-left" aria-hidden="true">(</span>
					<input
						type="tel"
						id="phone-area"
						class="kc-phone-area-input"
						maxlength="3"
						inputmode="numeric"
						pattern="[0-9]*"
						aria-label="Код города"
						<#if attribute.readOnly>disabled</#if>
					/>
					<span class="kc-phone-area-bracket kc-phone-area-bracket-right" aria-hidden="true">)</span>
				</div>
				<div class="kc-phone-main">
					<input
						type="tel"
						id="phone-main"
						class="kc-phone-main-input"
						inputmode="numeric"
						pattern="[0-9\-]*"
	...
	<#elseif attribute.name == "phoneNumber">
		<#-- Visible field only (no intl-tel-input) -->
		<span class="${properties.kcInputClass} <#if error?has_content>${properties.kcError}</#if>">
			<input type="tel" id="${attribute.name}" name="${attribute.name}" value="${(value!'')}" class="${properties.kcInputClass!}" 
				aria-invalid="<#if messagesPerField.existsError('${attribute.name}')>true</#if>"
				<#if attribute.readOnly>disabled</#if>
